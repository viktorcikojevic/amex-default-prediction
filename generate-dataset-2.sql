create or replace table `big-query-study-338516.kaggle_amex_dataset.train_data_labels_100percent_processed_2` as
...
num_features_last_values as (
  ...
  FIRST_VALUE(B_30 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as B_30_last_value,
  FIRST_VALUE(B_38 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as B_38_last_value,
  FIRST_VALUE(D_114 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_114_last_value,
  FIRST_VALUE(D_116 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_116_last_value,
  FIRST_VALUE(D_117 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_117_last_value,
  FIRST_VALUE(D_120 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_120_last_value,
  FIRST_VALUE(D_126 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_126_last_value,
  FIRST_VALUE(D_63 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_63_last_value,
  FIRST_VALUE(D_64 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_64_last_value,
  FIRST_VALUE(D_66 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_66_last_value,
  -- numerical features
