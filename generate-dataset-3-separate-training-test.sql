create or replace table `big-query-study-338516.kaggle_amex_dataset.train_data_labels_100percent_processed_3` as
...
with num_features_trends as (
  ...
  from `big-query-study-338516.kaggle_amex_dataset.train_data_labels`
),

num_features_last_values as (
  select 
  customer_ID,
  FIRST_VALUE(target IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as target,
  -- categorical features
  CAST(FIRST_VALUE(B_30 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as B_30_last_value,
  CAST(FIRST_VALUE(B_38 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as B_38_last_value,
  CAST(FIRST_VALUE(D_114 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_114_last_value,
  CAST(FIRST_VALUE(D_116 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_116_last_value,
  CAST(FIRST_VALUE(D_117 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_117_last_value,
  CAST(FIRST_VALUE(D_120 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_120_last_value,
  CAST(FIRST_VALUE(D_126 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_126_last_value,
  CAST(FIRST_VALUE(D_63 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_63_last_value,
  CAST(FIRST_VALUE(D_64 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_64_last_value,
  CAST(FIRST_VALUE(D_66 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) AS STRING) as D_66_last_value,
  -- numerical features
  FIRST_VALUE(D_68 IGNORE NULLS) OVER(PARTITION BY customer_ID ORDER BY S_2 desc) as D_68_last_value,
  FIRST_VALUE(P_2 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS P_2_last_value,
  FIRST_VALUE(D_39 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_39_last_value,
  FIRST_VALUE(B_1 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_1_last_value,
  FIRST_VALUE(B_2 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_2_last_value,
  FIRST_VALUE(R_1 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_1_last_value,
  FIRST_VALUE(S_3 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_3_last_value,
  FIRST_VALUE(D_41 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_41_last_value,
  FIRST_VALUE(B_3 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_3_last_value,
  FIRST_VALUE(D_42 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_42_last_value,
  FIRST_VALUE(D_43 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_43_last_value,
  FIRST_VALUE(D_44 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_44_last_value,
  FIRST_VALUE(B_4 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_4_last_value,
  FIRST_VALUE(D_45 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_45_last_value,
  FIRST_VALUE(B_5 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_5_last_value,
  FIRST_VALUE(R_2 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_2_last_value,
  FIRST_VALUE(D_46 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_46_last_value,
  FIRST_VALUE(D_47 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_47_last_value,
  FIRST_VALUE(D_48 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_48_last_value,
  FIRST_VALUE(D_49 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_49_last_value,
  FIRST_VALUE(B_6 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_6_last_value,
  FIRST_VALUE(B_7 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_7_last_value,
  FIRST_VALUE(B_8 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_8_last_value,
  FIRST_VALUE(D_50 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_50_last_value,
  FIRST_VALUE(D_51 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_51_last_value,
  FIRST_VALUE(B_9 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_9_last_value,
  FIRST_VALUE(R_3 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_3_last_value,
  FIRST_VALUE(D_52 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_52_last_value,
  FIRST_VALUE(P_3 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS P_3_last_value,
  FIRST_VALUE(B_10 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_10_last_value,
  FIRST_VALUE(D_53 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_53_last_value,
  FIRST_VALUE(S_5 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_5_last_value,
  FIRST_VALUE(B_11 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_11_last_value,
  FIRST_VALUE(S_6 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_6_last_value,
  FIRST_VALUE(D_54 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_54_last_value,
  FIRST_VALUE(R_4 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_4_last_value,
  FIRST_VALUE(S_7 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_7_last_value,
  FIRST_VALUE(B_12 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_12_last_value,
  FIRST_VALUE(S_8 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_8_last_value,
  FIRST_VALUE(D_55 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_55_last_value,
  FIRST_VALUE(D_56 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_56_last_value,
  FIRST_VALUE(B_13 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_13_last_value,
  FIRST_VALUE(R_5 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_5_last_value,
  FIRST_VALUE(D_58 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_58_last_value,
  FIRST_VALUE(S_9 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_9_last_value,
  FIRST_VALUE(B_14 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_14_last_value,
  FIRST_VALUE(D_59 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_59_last_value,
  FIRST_VALUE(D_60 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_60_last_value,
  FIRST_VALUE(D_61 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_61_last_value,
  FIRST_VALUE(B_15 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_15_last_value,
  FIRST_VALUE(S_11 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_11_last_value,
  FIRST_VALUE(D_62 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_62_last_value,
  FIRST_VALUE(D_65 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_65_last_value,
  FIRST_VALUE(B_16 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_16_last_value,
  FIRST_VALUE(B_17 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_17_last_value,
  FIRST_VALUE(B_18 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_18_last_value,
  FIRST_VALUE(B_19 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_19_last_value,
  FIRST_VALUE(B_20 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_20_last_value,
  FIRST_VALUE(S_12 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_12_last_value,
  FIRST_VALUE(R_6 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_6_last_value,
  FIRST_VALUE(S_13 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_13_last_value,
  FIRST_VALUE(B_21 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_21_last_value,
  FIRST_VALUE(D_69 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_69_last_value,
  FIRST_VALUE(B_22 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_22_last_value,
  FIRST_VALUE(D_70 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_70_last_value,
  FIRST_VALUE(D_71 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_71_last_value,
  FIRST_VALUE(D_72 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_72_last_value,
  FIRST_VALUE(S_15 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_15_last_value,
  FIRST_VALUE(B_23 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_23_last_value,
  FIRST_VALUE(D_73 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_73_last_value,
  FIRST_VALUE(P_4 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS P_4_last_value,
  FIRST_VALUE(D_74 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_74_last_value,
  FIRST_VALUE(D_75 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_75_last_value,
  FIRST_VALUE(D_76 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_76_last_value,
  FIRST_VALUE(B_24 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_24_last_value,
  FIRST_VALUE(R_7 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_7_last_value,
  FIRST_VALUE(D_77 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_77_last_value,
  FIRST_VALUE(B_25 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_25_last_value,
  FIRST_VALUE(B_26 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_26_last_value,
  FIRST_VALUE(D_78 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_78_last_value,
  FIRST_VALUE(D_79 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_79_last_value,
  FIRST_VALUE(R_8 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_8_last_value,
  FIRST_VALUE(R_9 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_9_last_value,
  FIRST_VALUE(S_16 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_16_last_value,
  FIRST_VALUE(D_80 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_80_last_value,
  FIRST_VALUE(R_10 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_10_last_value,
  FIRST_VALUE(R_11 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_11_last_value,
  FIRST_VALUE(B_27 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_27_last_value,
  FIRST_VALUE(D_81 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_81_last_value,
  FIRST_VALUE(D_82 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_82_last_value,
  FIRST_VALUE(S_17 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_17_last_value,
  FIRST_VALUE(R_12 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_12_last_value,
  FIRST_VALUE(B_28 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_28_last_value,
  FIRST_VALUE(R_13 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_13_last_value,
  FIRST_VALUE(D_83 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_83_last_value,
  FIRST_VALUE(R_14 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_14_last_value,
  FIRST_VALUE(R_15 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_15_last_value,
  FIRST_VALUE(D_84 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_84_last_value,
  FIRST_VALUE(R_16 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_16_last_value,
  FIRST_VALUE(B_29 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_29_last_value,
  FIRST_VALUE(S_18 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_18_last_value,
  FIRST_VALUE(D_86 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_86_last_value,
  FIRST_VALUE(D_87 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_87_last_value,
  FIRST_VALUE(R_17 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_17_last_value,
  FIRST_VALUE(R_18 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_18_last_value,
  FIRST_VALUE(D_88 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_88_last_value,
  FIRST_VALUE(B_31 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_31_last_value,
  FIRST_VALUE(S_19 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_19_last_value,
  FIRST_VALUE(R_19 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_19_last_value,
  FIRST_VALUE(B_32 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_32_last_value,
  FIRST_VALUE(S_20 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_20_last_value,
  FIRST_VALUE(R_20 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_20_last_value,
  FIRST_VALUE(R_21 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_21_last_value,
  FIRST_VALUE(B_33 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_33_last_value,
  FIRST_VALUE(D_89 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_89_last_value,
  FIRST_VALUE(R_22 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_22_last_value,
  FIRST_VALUE(R_23 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_23_last_value,
  FIRST_VALUE(D_91 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_91_last_value,
  FIRST_VALUE(D_92 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_92_last_value,
  FIRST_VALUE(D_93 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_93_last_value,
  FIRST_VALUE(D_94 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_94_last_value,
  FIRST_VALUE(R_24 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_24_last_value,
  FIRST_VALUE(R_25 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_25_last_value,
  FIRST_VALUE(D_96 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_96_last_value,
  FIRST_VALUE(S_22 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_22_last_value,
  FIRST_VALUE(S_23 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_23_last_value,
  FIRST_VALUE(S_24 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_24_last_value,
  FIRST_VALUE(S_25 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_25_last_value,
  FIRST_VALUE(S_26 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_26_last_value,
  FIRST_VALUE(D_102 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_102_last_value,
  FIRST_VALUE(D_103 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_103_last_value,
  FIRST_VALUE(D_104 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_104_last_value,
  FIRST_VALUE(D_105 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_105_last_value,
  FIRST_VALUE(D_106 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_106_last_value,
  FIRST_VALUE(D_107 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_107_last_value,
  FIRST_VALUE(B_36 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_36_last_value,
  FIRST_VALUE(B_37 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_37_last_value,
  FIRST_VALUE(R_26 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_26_last_value,
  FIRST_VALUE(R_27 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_27_last_value,
  FIRST_VALUE(D_108 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_108_last_value,
  FIRST_VALUE(D_109 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_109_last_value,
  FIRST_VALUE(D_110 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_110_last_value,
  FIRST_VALUE(D_111 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_111_last_value,
  FIRST_VALUE(B_39 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_39_last_value,
  FIRST_VALUE(D_112 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_112_last_value,
  FIRST_VALUE(B_40 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_40_last_value,
  FIRST_VALUE(S_27 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS S_27_last_value,
  FIRST_VALUE(D_113 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_113_last_value,
  FIRST_VALUE(D_115 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_115_last_value,
  FIRST_VALUE(D_118 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_118_last_value,
  FIRST_VALUE(D_119 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_119_last_value,
  FIRST_VALUE(D_121 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_121_last_value,
  FIRST_VALUE(D_122 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_122_last_value,
  FIRST_VALUE(D_123 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_123_last_value,
  FIRST_VALUE(D_124 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_124_last_value,
  FIRST_VALUE(D_125 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_125_last_value,
  FIRST_VALUE(D_127 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_127_last_value,
  FIRST_VALUE(D_128 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_128_last_value,
  FIRST_VALUE(D_129 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_129_last_value,
  FIRST_VALUE(B_41 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_41_last_value,
  FIRST_VALUE(B_42 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS B_42_last_value,
  FIRST_VALUE(D_130 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_130_last_value,
  FIRST_VALUE(D_131 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_131_last_value,
  FIRST_VALUE(D_132 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_132_last_value,
  FIRST_VALUE(D_133 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_133_last_value,
  FIRST_VALUE(R_28 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS R_28_last_value,
  FIRST_VALUE(D_134 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_134_last_value,
  FIRST_VALUE(D_135 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_135_last_value,
  FIRST_VALUE(D_136 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_136_last_value,
  FIRST_VALUE(D_137 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_137_last_value,
  FIRST_VALUE(D_138 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_138_last_value,
  FIRST_VALUE(D_139 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_139_last_value,
  FIRST_VALUE(D_140 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_140_last_value,
  FIRST_VALUE(D_141 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_141_last_value,
  FIRST_VALUE(D_142 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_142_last_value,
  FIRST_VALUE(D_143 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_143_last_value,
  FIRST_VALUE(D_144 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_144_last_value,
  FIRST_VALUE(D_145 IGNORE NULLS) OVER(partition by customer_ID ORDER BY S_2 desc) AS D_145_last_value
  from `big-query-study-338516.kaggle_amex_dataset.train_data_labels`
),
...
select 
  ...
  num_features_last_values.* EXCEPT(customer_ID), -- here are the targets
