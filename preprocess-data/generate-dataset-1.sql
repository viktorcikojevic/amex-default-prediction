create or replace table `big-query-study-338516.kaggle_amex_dataset.train_data_labels_100percent_processed_1` as
with cat_features AS (
  select customer_ID, B_30, B_38, D_114, D_116, D_117, D_120, D_126, D_63, D_64, D_66, D_68, target
  from 
  `big-query-study-338516.kaggle_amex_dataset.train_data_labels`
  group by customer_ID, B_30, B_38, D_114, D_116, D_117, D_120, D_126, D_63, D_64, D_66, D_68, target
),




num_features_trends as (
  select 
  customer_ID,
...
select 
  cat_features.* EXCEPT(target),
  num_features_agg_trends.* EXCEPT(customer_ID),
  num_agg_features_trends.* EXCEPT(customer_ID),
  cat_features.target
from cat_features
left join num_features_agg_trends
on cat_features.customer_ID = num_features_agg_trends.customer_ID
